...
{
public:
    DesktopWidgetPlugin(const QString & configId, ReadSettings * config)
        : m_config(config),
          m_configId(configId)
    {
    };
 ...
 */
typedef DesktopWidgetPlugin* (*DesktopWidgetInitFunction)(const QString & configId, ReadSettings * config);
...
#define EXPORT_RAZOR_DESKTOP_WIDGET_PLUGIN_H \
    extern "C" DesktopWidgetPlugin* init(const QString & configId, ReadSettings * config);

/*! Helper macro for define RazorPanelPlugin.
    Place this macro in your plugin source file.
 */
#define EXPORT_RAZOR_DESKTOP_WIDGET_PLUGIN_CPP(PLUGINCLASS)        \
    DesktopWidgetPlugin* init(const QString & configId, ReadSettings * config)              \
    {                                                       \
