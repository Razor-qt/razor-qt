 ...
 * @brief constructor
 */
RazorBarTask::RazorBarTask(RazorPanelPlugin* _owner, RazorPanel * panel)
    : QWidget(_owner),
      owner(_owner),
      m_panel(panel)
{
...
{
    active = xfitMan()->getActiveAppWindow();
    ...
    for (int i=0; i < taskMap.values().count(); i++)
      if (xfitMan()->requiresAttention(taskMap.keys().at(i)))
...
void RazorBarTask::updateTasks(QMap<Window, RazorTask*>* _list)
{
    ...
    while (newiter.hasNext())
    {
        ...
        if (m_panel->isHorizontal())
        {
            ...
            //item->setMaximumWidth(100);//owner->width()/qMax(_list->count(), 3));
